...
// ---- Chess TV PATCH BEGIN: per-architecture SIMD gate ----------------------
// SwiftPM cannot condition `-D` flags on the target architecture, only on the
// platform, and an Apple simulator build compiles both an arm64 and an x86_64
// slice from the same settings. The package therefore defines only
// SF_APPLE_ARCH_GATE and the real SIMD selection happens here, where every
// translation unit sees it: misc.h is the first Stockfish header pulled in by
// types.h, nnue_common.h and nnue/simd.h.
//   arm64  -> USE_NEON (8), plus USE_NEON_DOTPROD when the compiler advertises
//             the ARMv8.2 dot-product extension.
//   x86_64 -> nothing; the baseline x86_64 ABI guarantees only SSE2 and we do
//             not pass -m flags, so Stockfish falls back to its portable code.
#if defined(SF_APPLE_ARCH_GATE)
    #if defined(__ARM_NEON) || defined(__aarch64__) || defined(__arm64__)
        #if !defined(USE_NEON)
            #define USE_NEON 8
        #endif
