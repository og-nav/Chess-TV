...
final class FakeStreamer: SourceStreaming, @unchecked Sendable {   // @unchecked: the state below is lock-guarded
    ...
    func emit(_ event: TVEvent, to source: GameSource, isHistorical: Bool = false) {
        lock.withLock { continuations[source] }?.yield(.event(SourcedEvent(event: event, isHistorical: isHistorical)))
    }
...
@Suite("AppModel starts one source at a time")
@MainActor
struct AppModelSourceTests {

    private func makeModel() -> (AppModel, FakeStreamer) {
        ...
        return (AppModel(settings: settings, streamer: streamer, arenas: FakeArenas()), streamer)
    }
...
struct ToastAndOrientationTests {
    ...
    private func makeModel() -> AppModel {
        let settings = AppSettings(defaults: UserDefaults(suiteName: "ChessTVTests-\(UUID().uuidString)")!)
        return AppModel(settings: settings, streamer: FakeStreamer(), arenas: FakeArenas())
    }
    ...
    func toggleFlipFromTheGameScreen() {
        ...
        let model = AppModel(settings: AppSettings(defaults: defaults), streamer: FakeStreamer(), arenas: FakeArenas())
        #expect(model.boardOrientation == .white)
...
struct ArenaStandingsTests {
    ...
    private func makeModel(_ arenas: FakeArenas) -> (AppModel, FakeStreamer) {
        let streamer = FakeStreamer()
        let settings = AppSettings(defaults: UserDefaults(suiteName: "ChessTVTests-\(UUID().uuidString)")!)
        return (AppModel(settings: settings, streamer: streamer, arenas: arenas), streamer)
    }
    ...
    func backoff() {
        #expect(AppModel.arenaStandingsDelay(afterFailures: 0) == .seconds(10))
        #expect(AppModel.arenaStandingsDelay(afterFailures: 1) == .seconds(10))
        #expect(AppModel.arenaStandingsDelay(afterFailures: 2) == .seconds(20))
        #expect(AppModel.arenaStandingsDelay(afterFailures: 9) == AppModel.arenaStandingsMaxInterval)
    }
}
